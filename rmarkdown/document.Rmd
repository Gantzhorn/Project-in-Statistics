...
```{r}
eagleFit1$mle$horizontal_steps
eagleFit1$mle$vertical_steps
...
df <- data.frame(x = rep(x, N),
                 density = as.vector(pdf_values),
                 state = factor(rep(1:N, each = length(x))))

# Plot the density functions using ggplot2
ggplot(eagleData, aes(x = horizontal_steps)) +
  geom_histogram(aes(y = ..density..),col = "black", fill = "gray",alpha = .6)

ggplot(eagleData) +
  geom_histogram(aes(x = horizontal_steps, after_stat(density)), fill = "gray", col = "black") +
  geom_density(data = df, aes(x = x, y = density, color = state),
