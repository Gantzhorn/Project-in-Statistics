...
lat_range <- range(eagleDataOriginal$Latitude) + c(-7.5, 7.5)
lon_range <- range(eagleDataOriginal$Longitude) + c(-7.5, 7.5)

# Get map data for the specified range
...
ggplot() +
  geom_polygon(data = map_data, aes(x = long, y = lat, group = group), fill = "lightgray", color = "gray50") +
  geom_point(data = eagleDataOriginal, aes(x = Longitude, y = Latitude),
             color = proj_palette[1], size = .3) +
  xlab("Longitude") +
  ylab("Latitude") +
  ggtitle("Longitude and Latitude on World Map") +
  theme_map() +
...
ggplot() + 
  ...
  geom_density(data = filter(df, state != "Total"), aes(x = x, y = density, color = state),
               stat = "identity", lwd = 1, alpha = 1.5) +
  xlab("x") +
  ylab("Density") +
...
#mapview::mapview(favourite_birdTibble, xcol = "Longitude", ycol = "Latitude", crs = 4269, grid = #FALSE, zcol = "decodedstates")
```
...
ggmap(favoriteBirdMap)+
  geom_line(data = favourite_birdTibble, aes(x = Longitude, y = Latitude, group = Segment_ID),
            lwd = 1, col = "black") + 
